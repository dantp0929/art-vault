...
module.exports = {
  ...
  async execute (interaction: any, dbClient: Client) {
    try {
      ...
      const existing = await dbClient.query('SELECT discord_link FROM public.submissions WHERE external_link = $1', [`${externalLink}`])
      ...
      const reply = await interaction.reply({
        content: tags ? `${externalLink} - ${tags}` : externalLink,
        fetchReply: true
      })
      ...
      const submission = await dbClient.query('INSERT INTO public.submissions(submitter, external_link, discord_link, created_on, updated_on) VALUES ($1, $2, $3, $4, $5) RETURNING id',
      ...
      tagArray.forEach(async (tag: string) => {
        ...
        await dbClient.query('INSERT INTO public.submission_tags(submission_id, tag) VALUES($1, $2)',
          [`${submission.rows[0].id}`, `${tag}`]
        )
      })
